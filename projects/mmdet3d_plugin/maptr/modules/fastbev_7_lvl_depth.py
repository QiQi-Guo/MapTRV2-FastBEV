...
class FastBEV(nn.Module):
    ...
    def extract_feat(self,
        ...
        B, N, D, H, W, C = img_feats.shape
        stride_i = math.ceil(img_metas[0]['img_shape'][0][1] / W)  # P4 880 / 32 = 27.5
        volumes = []
        for batch_id, img_meta in enumerate(img_metas):
            feat_i = img_feats[batch_id]  # [nv, c, h, w]
            # height = img_feats.shape[-2]
        ...
        x = torch.stack(volumes)  # [bs, lvl*seq*c, vx, vy, vz]
    ...
    def forward(self, img_feats, img_metas):
        ...
        x, depth = self.get_cam_feats(img_feats, mlp_input)
...
def backproject_inplace(features, points, img_meta, stride, noise):
    ...
    '''
    n_images, n_depth, height, width, n_channels = features.shape
    features = features.permute(0, 4, 2, 3, 1).contiguous()
    ...
    for i in range(n_images):
        volume[:, valid[i]] = linear_interpolate(i, x, y, z, valid, features)

    volume = volume.view(n_channels, n_x_voxels, n_y_voxels, n_z_voxels)
    volume = volume.sum(dim=3, keepdims=True)  # compress height
    return volume

def linear_interpolate(i, x, y, z, valid, features):
    depth_idx = ((z - 1) // 0.5).to(torch.int64)
    depth_1 = depth_idx * 0.5 + 1
    depth_2 = (depth_idx + 1) * 0.5 + 1
    depth_1_valid = depth_1[i, valid[i]]
    depth_2_valid = depth_2[i, valid[i]]
    z_valid = z[i, valid[i]]
    feat_1 = (depth_2_valid - z_valid) / (depth_2_valid - depth_1_valid) * features[i, :, y[i, valid[i]], x[i, valid[i]], depth_idx[i, valid[i]]]
    feat_2 = (z_valid - depth_1_valid) / (depth_2_valid - depth_1_valid) * features[i, :, y[i, valid[i]], x[i, valid[i]], (depth_idx[i, valid[i]]+1)]
